...
Public Class ProjectDALC
        ...
        Try
            ...
            For Each row As DataRow In data.Rows
                ...
                If fromindicadores = 0 Then
                    ...
                    objProject.antecedent = IIf(Not IsDBNull(row("antecedent")), row("antecedent"), Nothing)
                    ...
                    objProject.zonedescription = IIf(Not IsDBNull(row("zonedescription")), row("zonedescription"), Nothing)
                    objProject.population = row("population")
                    objProject.strategicdescription = IIf(Not IsDBNull(row("strategicdescription")), row("strategicdescription"), Nothing)
                    objProject.results = row("results")
                    objProject.source = IIf(Not IsDBNull(row("source")), row("source"), Nothing)
                    objProject.purpose = IIf(Not IsDBNull(row("purpose")), row("purpose"), Nothing)
                    objProject.totalcost = IIf(Not IsDBNull(row("totalcost")), row("totalcost"), Nothing)
                    objProject.fsccontribution = IIf(Not IsDBNull(row("fsccontribution")), row("fsccontribution"), Nothing)
                    objProject.counterpartvalue = IIf(Not IsDBNull(row("counterpartvalue")), row("counterpartvalue"), Nothing)
                    ...
                    objProject.projectlocationlist = objProjectLocationDALC.getList(objApplicationCredentials, idproject:=objProject.id)
                    'objProject.thirdbyprojectlist = objThirdByProjectDALC.getList(objApplicationCredentials, idproject:=objProject.idKey)
                    objProject.operatorbyprojectlist = objOperatorByProjectDALC.getList(objApplicationCredentials, idproject:=objProject.id)
                    objProject.ProgramComponentbyprojectlist = objProgramComponentByProjectDALC.getList(objApplicationCredentials, idproject:=objProject.id)
                    objProject.forumlist = objForumDALC.getList(objApplicationCredentials, idproject:=objProject.idKey)
    ...
    Optional ByVal code As String = "" _
        ...
        Try
            ...
            sql.Append("select id, name, typeapproval, code, ")
            sql.Append("convert(varchar,id) + '_' + Name as codigo ")
            sql.Append("from Project where typeapproval = 1 ")
